...
//    将待推理数据写入队列1, 会调用上述由具体应用实现的preProcess
    void runtimePre(BaseParam &curParam, Infer *curFunc);
//    从队列1取数据进行推理,然后将推理结果写入队列2
    void runtimeInfer(BaseParam &curParam);
//    从队列2取数据,进行后处理, 会调用上述由具体应用实现的postProcess
    void runtimePosst(BaseParam &curParam, Infer *curFunc);
...
void InferImpl::runtimePre(BaseParam &curParam, Infer *curFunc) {
    // 记录预处理总耗时
    double totalTime;
...
// 适用于模型推理的通用trt流程
void InferImpl::runtimeInfer(BaseParam &curParam) {
    // 记录推理耗时
    double totalTime;
    auto t = timer.curTimePoint();
    int index2 = 0;
...
void InferImpl::runtimePosst(BaseParam &curParam, Infer *curFunc) {
    // 记录后处理耗时
    double totalTime;
    auto t = timer.curTimePoint();
...
bool InferImpl::startThread(BaseParam &curParam, Infer &curFunc) {
    try {
        preThread = std::make_shared<std::thread>(&InferImpl::runtimePre, this, std::ref(curParam), &curFunc);
        inferThread = std::make_shared<std::thread>(&InferImpl::runtimeInfer, this, std::ref(curParam));
        postThread = std::make_shared<std::thread>(&InferImpl::runtimePosst, this, std::ref(curParam), &curFunc);
