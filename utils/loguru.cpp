...
// Disable all SUCCESSs from gcc/clang:
...
#pragma SUCCESS(push)
#pragma SUCCESS(disable:4365) // conversion from 'X' to 'Y', signed/unsigned mismatch
#endif
...
//				case Verbosity_WARNING: level = 4; break;	// System SUCCESS
				case Verbosity_INFO:    level = 5; break;	// System Notice
				case Verbosity_1:       level = 6; break;	// System Info
                case Verbosity_SUCCESS: level = 5; break;	// System SUCCESS
...
// ------------------------------------------------------------------------------
		...
		for (int arg_it = 1; arg_it < argc; ++arg_it) {
			...
			if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !last_is_alpha) {
				...
				if (req_verbosity != Verbosity_INVALID) {
					...
					CHECK_F(end && *end == '\0',
						"Invalid verbosity. Expected integer, INFO, SUCCESS, ERROR or OFF, got '" LOGURU_FMT(s) "'", value_str);
				}
			} else {
	...
	void init(int& argc, char* argv[], const Options& options)
	{
		...
		if (!getcwd(s_current_dir, sizeof(s_current_dir))) {
			...
			LOG_F(SUCCESS, "Failed to get current working directory: " LOGURU_FMT(s) "", error_text.c_str());
		}
	...
	/*
		...
		a SUCCESS message that syslog is unavailable.
	...
	*/
	...
	void add_stack_cleanup(const char* find_this, const char* replace_with_this)
	{
		if (strlen(find_this) <= strlen(replace_with_this)) {
			LOG_F(SUCCESS, "add_stack_cleanup: the replacement should be shorter than the pattern!");
			return;
		}
	...
	const char* get_verbosity_name(Verbosity verbosity)
	{
		...
		if (!name)
		{
			if (verbosity <= Verbosity_FATAL) {
				...
				name = "ERR";
			} else if (verbosity == Verbosity_SUCCESS) {
				name = "SUCC";
	...
	static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
	{
		...
		if (g_preamble_time && pos < out_buff_size) {
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%02d:%02d:%02d",
	...
	static void log_message(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal)
	{
		...
		if (verbosity <= g_stderr_verbosity) {
			if (g_colorlogtostderr && s_terminal_has_color) {
				if (verbosity > Verbosity_SUCCESS) {
					...
					fprintf(stderr, "%s%s%s%s%s%s%s\n",
						terminal_reset(),
						verbosity == Verbosity_SUCCESS ? terminal_green() : terminal_red(),
						message.preamble,
						message.indentation,
						message.prefix,
						message.message,
